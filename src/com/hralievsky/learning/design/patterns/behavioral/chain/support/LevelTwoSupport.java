...
public class LevelTwoSupport extends ChainedSupport {

    @Override
    protected void process(Ticket ticket) {
        System.out.println(LevelTwoSupport.class.getSimpleName() + " processing ticket with priorities " + ticket.getPriorities().name() + ": " + ticket.getDescription());
    }

    @Override
    protected boolean canHandle(Ticket ticket) {
        return ticket.getPriorities() == TicketPriorities.HIGH;
    }
}
